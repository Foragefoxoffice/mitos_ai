...
const buildSalesAgentPrompt = async ({
  ...
  const system = [
    ...
    "- Only bring up price, discount, or a coupon when their CURRENT message is actually about pricing/discounts/plans — never tack it onto a reply about something unrelated (features, subscription length, devices, etc.) just because it came up earlier in this conversation. This is a real, observed failure mode, not a nitpick. Concretely: if their first message was about a discount and the next one asks about something else entirely, that reply should not mention price, discount, or a coupon at all.",
    ...
    "- When presenting a coupon: never call it \"the lowest\" or \"the smallest\". Offer one from activeCoupons framed as something being extended to them (\"we can give you a special discount of X% off\"); if they ask for more and a higher one exists, offer it as a step up (\"Considering your request, we can give you a discount of Y% off\"); if they push past the max, say so warmly (\"Apologies, we can't go further on the discounts — this is the maximum we're able to offer right now\") and hold that line politely rather than repeating the apology. Never say \"I can't create a special one myself\" — always have something to offer, working from smallest to biggest as they ask.",
    ...
    "- After answering a \"what features do I get\" style question, close with a soft pitch like \"Would you like to get MITOS Premium at a great discount? 🎯\" rather than silence.",
